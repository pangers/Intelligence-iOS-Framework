...
class IntelligenceAnalyticsTestCase: IntelligenceBaseTestCase {
    ...
    func testAnalyticsModule() {
        ...
        analytics.eventQueue = EventQueue { (events, completion) -> Void in
            completion(nil)
        }
        let storage = MockTimeTrackerStorage()
        analytics.timeTracker = TimeTracker(storage: storage, callback: { (event) -> Void in
        })
    ...
    func testEventQueue() {
        let myQueue = EventQueue { (events, completion) -> Void in
