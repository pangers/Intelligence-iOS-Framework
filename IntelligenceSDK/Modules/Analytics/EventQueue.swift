...
typealias EventQueueCallback = (JSONDictionaryArray, @escaping (NSError?) -> Void) -> Void
...
class EventQueue: NSObject {
    ...
    lazy var eventArray = JSONDictionaryArray()
    ...
    @discardableResult
    func fire(withCompletion completion: ((_ error: NSError?) -> Void)?) -> Bool {
        var result = false
