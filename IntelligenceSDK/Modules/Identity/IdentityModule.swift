...
typealias InstallationCallback = (Installation?, NSError?) -> Void
...
final class IdentityModule: IntelligenceModule, IdentityModuleProtocol {
    ...
    override func startup(completion: @escaping (Bool) -> Void) {
        ...
        super.startup { [weak network, weak configuration] (success) -> Void in
            if !success {
            ...
            network.getPipeline(forOAuth: oauth, configuration: configuration) { [weak self] (applicationPipeline) -> Void in
                ...
                applicationPipeline.callback = { [weak self] (returnedOperation) in
                    ...
                    identity.createInstallation(callback: { (_, error) in
                        if nil == error {
                            EventTypes.ApplicationInstall.saveToUserDefault(Obj: true)
                        }
                    })

                    identity.updateInstallation(callback: { (_, error) in
                        if nil == error {
    ...
    @objc func login(with username: String, password: String, callback: @escaping UserCallback) {
        ...
        pipeline.callback = { [weak self] (returnedOperation: IntelligenceAPIOperation) -> Void in
    ...
    func getMe(oauth: IntelligenceOAuthProtocol, callback: @escaping UserCallback) {

        let operation = GetUserMeRequestOperation(oauth: oauth, configuration: configuration, network: network, callback: { (returnedOperation: IntelligenceAPIOperation) -> Void in
    ...
    func registerDeviceToken(with data: Data, callback: @escaping RegisterDeviceTokenCallback) {
        ...
        unregisterDeviceTokenOnBehalf(token: token) { [weak self] (_) -> Void in
    ...
    private func registerDeviceToken(token: String, callback: @escaping RegisterDeviceTokenCallback) {
        ...
        if token.count == 0 || token.lengthOfBytes(using: String.Encoding.utf8) == 0 {
        ...
        let operation = CreateIdentifierRequestOperation(token: token,
            ...
            network: network,
            callback: {
                (returnedOperation: IntelligenceAPIOperation) -> Void in
    ...
    func createInstallation(callback: InstallationCallback? = nil) {
        if !installation.isNewInstallation {
            callback?(installation, NSError(code: InstallationError.alreadyInstalledError.rawValue))
            return
        }

            let operation = CreateInstallationRequestOperation(installation: installation, oauth: network.oauthProvider.applicationOAuth, configuration: configuration, network: network, callback: { (returnedOperation: IntelligenceAPIOperation) -> Void in
    ...
    /// - Parameters:
    ///     - callback: Optionally provide a callback to fire on completion.
    func updateInstallation(callback: InstallationCallback? = nil) {
        ...
        // If this call fails, it will retry again the next time we open the app.
            ...
            let operation = UpdateInstallationRequestOperation(installation: installation, oauth: network.oauthProvider.applicationOAuth, configuration: configuration, network: network, callback: { (returnedOperation: IntelligenceAPIOperation) -> Void in
