...
extension Sequence {
    ...
    */
    func forEachInMainThread(body: @escaping (Self.Iterator.Element) -> Void) {
    ...
    */
    func forEach(asyncInQueue queue: DispatchQueue, body: @escaping(Self.Iterator.Element) -> Void) {
        queue.async {
            self.forEach(body)
        }
    }
