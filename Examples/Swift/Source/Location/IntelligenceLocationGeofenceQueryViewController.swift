...
class IntelligenceLocationGeofenceQueryViewController: UIViewController {
    ...
    @IBAction func didTapSave(sender: AnyObject) {
        ...
        if let text = latitudeText.text, text.count > 0, let latitudeFromText = Double(text) {
        ...
        if let text = longitudeText.text, text.count > 0, let longitudeFromText = Double(text) {
        ...
        if let text = radiusText.text, text.count > 0, let radiusFromText = Double(text) {
        ...
        query.pageSize = (pageSizeText.text ?? "").count > 0 ? Int(pageSizeText.text!) : nil
        query.pageNumber = (pageText.text ?? "").count > 0 ? Int(pageText.text!) : nil
