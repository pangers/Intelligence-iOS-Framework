...
class IntelligenceAnalyticsTestCase: IntelligenceBaseTestCase {
    ...
    override func tearDown() {
        super.tearDown()
        let queue = EventQueue { (_, _: (_ error: NSError?) -> Void) -> Void in
    ...
    func testAnalyticsModule() {
        ...
        analytics.eventQueue = EventQueue { (_, completion) -> Void in
            completion(nil)
        }
        let storage = MockTimeTrackerStorage()
        analytics.timeTracker = TimeTracker(storage: storage, callback: { (_) -> Void in
        })
      ...
      let dictionary = analytics.eventQueue!.eventArray.first
    ...
    func testTimeTracker() {
        ...
        let storage = MockTimeTrackerStorage()
        storage.duration = 10
        var timeTracker: TimeTracker? = TimeTracker(storage: storage, callback: { (event) -> Void in
    ...
    func testEventQueue() {
        let myQueue = EventQueue { (_, _) -> Void in
    ...
    func mockSendAnalytics(_ status: HTTPStatusCode = .success, event: Event, eventsJSONResponse: JSONDictionary? = nil, completion: @escaping (_ error: NSError?) -> Void) {
        let analytics = (intelligence?.analytics as! AnalyticsModule)
	...
	func testScreenViewedSuccess() {
        ...
        mockOAuthProvider.fakeLoggedIn(mockOAuthProvider.loggedInUserOAuth, fakeUser: fakeUser)
        mockSendAnalytics(event: event) { (error) -> Void in
            XCTAssertNil(error, "Expected success")
            expectCallback.fulfill()
        }
    ...
    func testAnalyticsError400InvalidRequest() {
        ...
        analytics.sendEvents(events: []) { (error) -> Void in
    ...
    func testEventsQueueFire() {
        let queue = EventQueue { (events, completion: (_ error: NSError?) -> Void) -> Void in
    ...
    func test101EventsInQueueRequiresTwoCalls() {
        ...
        queue.clearEvents() // Empty file first
        ensureJSONIncludesMandatoryPopulatedData(eventJSON)
        (0...queue.maxEvents).forEach({ _ -> Void in queue.enqueueEvent(event: eventJSON) })
